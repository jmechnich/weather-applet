...
class OWMParser:
    ...
    def getWeatherByID(self, locid, keyword="weather"):
        ...
        if not len(w) and keyword == "weather":
            syslog.syslog(syslog.LOG_WARNING, "WARN   Falling back to 'group' API call")
    ...
    def getWeather(self, url, args):
        data = {}
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  %s" % url + urllib.parse.urlencode(args)
        )
        ...
        j = json.load(f)
        if ("cod" in j and int(j["cod"]) != 200) or ("cnt" in j and int(j["cnt"]) == 0):
    ...
    def connect(self, url):
        ...
        for i in range(retries):
            try:
                ...
                syslog.syslog(syslog.LOG_DEBUG, "DEBUG  owm connect error: %s" % str(e))
                syslog.syslog(
                    syslog.LOG_WARNING,
            ...
            syslog.syslog(
                syslog.LOG_ERR, "ERROR  failed to retrieve data from '%s'" % url
            )
