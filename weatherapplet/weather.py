...
import json, os, syslog, urllib.request
from datetime import datetime, timedelta

from PyQt6.QtCore import Qt, QSignalMapper
from PyQt6.QtGui import QActionGroup, QFont, QIcon, QPainter, QPixmap
...
class WeatherIndicator(Indicator):
    ...
    def __init__(self):
        ...
        syslog.syslog(
            syslog.LOG_DEBUG,
            f"DEBUG  initializing WeatherIndicator )app: {Application.instance()}",
        )
        ...
        self.initStats()
        self.initWidgets()
        Application.instance().sigusr1.connect(self.restart)

    def initVars(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  initializing variables")

        self.data = {}
        self.mode_actions = {}
        self.mode = int(Application.settingsValue("mode", Modes.Now))
        self.location = int(Application.settingsValue("location", Location.Auto))
        ...
        self.cachedir = os.path.join(
            os.environ["HOME"], ".cache", str(Application.instance().applicationName())
        )
        self.prefs = None
        self.icon = None
    ...
    def initOwm(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  initializing owm parser")
        self.owm = OWMParser(self.apikey if len(self.apikey) else None)

    def initContextMenu(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  initializing context menu")
        m = QMenu()
        g = QActionGroup(m)
        g.setExclusive(True)
        sm = QSignalMapper(m)
        ...
        sm.mappedInt.connect(self.setMode)
        m.addSeparator()
        m.addAction(QIcon.fromTheme("view-refresh"), "&Update", self.updateAll)
        m.addAction(
            QIcon.fromTheme("preferences-other"), "&Options", self.showPreferences
        )
        m.addAction(
            QIcon.fromTheme("application-exit"), "&Quit", Application.instance().quit
        )
        self.systray.setContextMenu(m)

    def initStats(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  initializing stats")
    ...
    def initWidgets(self):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  initializing widgets")
    ...
    def setLocationFromIP(self):
        ...
        try:
            ...
            syslog.syslog(
                syslog.LOG_ERR, "ERROR  weather setLocationFromIP failed: %s" % str(e)
            )

            return False
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  weather setLocationFromIP %f %f" % (lon, lat)
        )

        return self.setLocationFromCoord(lon, lat)

    def setLocationFromName(self, s):
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  weather setLocationFromName %s" % s)
        if self.locationName().startswith(s):
            return False
        ...
        self.locvalid = True
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  weather setLocationFromName: %d" % self.locid
        )
        return True

    def setLocationFromCoord(self, lon, lat):
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  weather setLocationFromCoord %f %f" % (lon, lat)
        )
        if lon in self.data and lat in self.data:
            if self.data["lon"] == lon and self.data["lat"] == lat:
                syslog.syslog(syslog.LOG_DEBUG, "DEBUG   data already up to date")
                return False
        locid = self.owm.findClosestID(lon, lat)
        if locid == -1:
            syslog.syslog(syslog.LOG_DEBUG, "DEBUG   error finding closest OWM ID")
            return False
        if self.locid == locid and len(self.data):
            return False
        self.locid = locid
        self.locvalid = True
        syslog.syslog(
            syslog.LOG_DEBUG, "DEBUG  weather setLocationFromCoord: %d" % self.locid
        )
        return True
    ...
    def updateLocation(self):
        ...
        self.updateAll()
        if (
            self.location != Location.Auto
            and self.locid != -1
            and self.locvalid == True
        ):
    ...
    def setMode(self, m):
        ...
        if self.splash:
            syslog.syslog(syslog.LOG_DEBUG, "DEBUG  weather setMode updateSplash")
    ...
    def systrayClicked(self, reason):
        syslog.syslog(
            syslog.LOG_DEBUG, f"DEBUG  weather systrayClicked '{reason.name}'"
        )
        if (
    ...
    def updateAll(self):
        syslog.syslog(
            syslog.LOG_INFO, "INFO   updateAll at %s" % datetime.now().isoformat()
        )
        self.data.clear()
        try:
            self.updateWeather()
    ...
    def updateWeather(self):
        ...
        if not len(data):
            syslog.syslog(
                syslog.LOG_WARNING, "WARN  could not get weather for '%d'" % self.locid
            )
            if not self.locvalid:
                self.locid = -1
            return
    ...
    def updateForecast(self):
        ...
        if not len(data):
            syslog.syslog(
                syslog.LOG_WARNING, "WARN  could not get forecast for '%d'" % self.locid
            )
            if not self.locvalid:
                self.locid = -1
            return
    ...
    def updateIcon(self):
        ...
        if not len(self.data):
            ...
            p.drawText(
                0, 0, pix.width(), pix.height(), Qt.AlignmentFlag.AlignCenter, "ERR"
            )
            p.end()
        ...
        p.drawText(
            ...
            "%d\xb0C" % round(self.data["temp"]),
        )
        p.end()
        self.icon = QIcon(pix)
        self.systray.setIcon(self.icon)
        if self.prefs:
            self.prefs.setWindowIcon(self.icon)
