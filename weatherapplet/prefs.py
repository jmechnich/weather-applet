from weatherapplet.utils import Location
...
class Preferences(QDialog):
    ...
    def init(self):
        ...
        self.apikey.returnPressed.connect(lambda: self.i.setAPIKey(self.apikey.text()))
        ...
        coord_val = QRegularExpressionValidator(
            QRegularExpression("[\d\.]+\s*,\s*[\d\.]+"), self
        )
    ...
    def selectLocation(self, b):
        ...
        syslog.syslog(syslog.LOG_DEBUG, "DEBUG  prefs selectLocation %d" % selected)
    ...
    def setLocationFromIP(self):
        ...
        if not self.i.setLocationFromIP():
            return
    ...
    def setLocationFromName(self):
        ...
        if not self.i.setLocationFromName(str(self.location[Location.Name].text())):
            return
        self.i.updateLocation()
        self.location[Location.Name].setText(self.i.locationName())
        self.location[Location.Coord].setText(self.i.locationCoord())
    ...
    def setLocationFromCoord(self):
        ...
        if not self.i.setLocationFromCoord(float(l[0]), float(l[1])):
            return
        self.i.updateLocation()
        self.location[Location.Name].setText(self.i.locationName())
        self.location[Location.Coord].setText(self.i.locationCoord())
